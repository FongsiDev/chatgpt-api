...
var ChatGPTUnofficialProxyAPI = class {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        await fetch(`${url}/${conversationId}`, {
          ...
          method: "PATCH"
        }).then((x) => x.text()).then(resolve).catch(reject);
      } catch (err) {
        reject(err);
      }
    });
  ...
  async genTitle(event) {
    ...
    if (!event || !event.conversationId || !event.messageId) {
      return null;
    }
    const conversationId = event.conversationId;
    const messageId = event.messageId;
    const baseUrl = this._apiReverseProxyUrl;
    ...
    try {
      const ret = await this._fetch(url, opts);
